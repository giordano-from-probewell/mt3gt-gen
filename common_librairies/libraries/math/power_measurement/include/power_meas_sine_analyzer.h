...
// $Release Date: Thu Jan 26 16:24:44 CST 2023 $
// $Copyright:
...
#define SAMPLE_RATE   (ADS_FREQ / 512.0f)
#define DELTA_UWH     (1e-6f  * 3600.0f * SAMPLE_RATE)  // energy by sample = 1µWh
#define DELTA_WH      (1.0f   * 3600.0f * SAMPLE_RATE)  // energy 1Wh
...
#define V_RMS_IIR_ALPHA 0.2f
#define I_RMS_IIR_ALPHA 0.2f
...
typedef volatile struct {
    ...
    int32_t  nShiftSamples;
    int32_t  nShiftSamplesCnt;
...
//! \return None
//!
static inline void POWER_MEAS_SINE_ANALYZER_run(POWER_MEAS_SINE_ANALYZER *v, POWER_MEAS_SINE_ANALYZER *ref)
{

    v->vNorm = fabsf(v->v);
    v->iNorm = fabsf(v->i);
    if(v->nSamplesMin < v->nSamples ){ //Without this the rms could be calculated over 2 cycles
    ...
    if((v->prevSign != v->currSign) && (v->currSign == 1))
    {
        ...
        {
            v->nShiftSamples = v->nShiftSamplesCnt;
            v->zcd=1;
            v->samples_per_cycle = v->nSamples;
            v->inverse_nSamples = (1.0f)/(v->nSamples);
            v->sqrt_inverse_nSamples = CLAsqrt(v->inverse_nSamples);
            v->vAvg = (v->vSum*v->inverse_nSamples);
            v->vRms = CLAsqrt(v->vSqrSum)*v->sqrt_inverse_nSamples;
            if(v->vRms>1.0)
                v->no_voltage = 0;
            v->iRms = CLAsqrt(v->iSqrSum)*v->sqrt_inverse_nSamples;
            ...
            v->vIir = (1.0f - V_RMS_IIR_ALPHA) * v->vIir + V_RMS_IIR_ALPHA * v->vRms;
            v->iIir = (1.0f - I_RMS_IIR_ALPHA) * v->iIir + I_RMS_IIR_ALPHA * v->iRms;
            ...
            if(v->slewPowerUpdate >= 100)
            {
                ...
                v->vaSumMul = 0;
                v->powerFactor=CLAdiv(v->pRms,v->vaRms);
