...
#define _CMD_ANSWER         0xA101 //if set to 0xA00D like the others, weird bug where 0xA00A is read on the linux

int16_t cmd_get_frequency(uint8_t *error_category, uint8_t *error_code,uint16_t *command, uint8_t *data, uint16_t *size)
{
    ...
    //float32_to_uint8_t(data, app.generation.voltage.ref.rgen.freq);
